...
function modals()
{
    ...
    $('.modalTarget').click(function(e){
        ...
        $.get(e.target.href,function(response){
            $('#myModal .modal-header h4').text($(e.target).text());
...
function skcms_ckfinder_popup_input()
{
    ...
    //        }
    //    );
    $('[data-type=elfinder-input-field]').each(function(){
        var img = $('<img />');
        var $elem = $(this)
        img.addClass('elfinderImg').attr('src',$(this).val());
        $(this).after(img);
        img.on('click',function(){$elem.trigger('click')});
    });

    $('[data-type=elfinder-input-field]').change(function(){
...
function addPrototypeForm()
{
    $('[data-prototype]').each(function(){
        ...
        var $addTagLink = $('<a href="#" class="add_entity_link btn pink-btn">Ajouter</a>');
